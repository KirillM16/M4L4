...
# Словарь для хранения избранных фильмов пользователей:
favorites_dict = {}
...
def main_markup():
    ...
    markup.add(KeyboardButton('/random'), KeyboardButton('/favorites'), KeyboardButton('/random_favorite'))
    return markup
...
/random_favorite - случайный фильм из избранного
...
@bot.message_handler(commands=['random_favorite'])
def random_favorite(message):
    user_id = message.from_user.id
    
    if user_id not in favorites_dict or len(favorites_dict[user_id]) == 0:
        bot.send_message(message.chat.id, "У вас нет избранных фильмов! Добавьте фильмы через кнопку ⭐")
        return
    ...
    try:
        ...
        with con:
            cur = con.cursor()
            movie_id = random.choice(favorites_dict[user_id])
            cur.execute("SELECT * FROM movies WHERE id = ?", (movie_id,))
            row = cur.fetchone()
            
            if row:
                bot.send_message(message.chat.id, "🎲 Случайный фильм из вашего избранного:")
...
def show_favorites(message):
    ...
    if user_id not in favorites_dict or len(favorites_dict[user_id]) == 0:
        bot.send_message(message.chat.id, "У вас пока нет избранных фильмов. Добавьте фильмы через кнопку ⭐")
        return
